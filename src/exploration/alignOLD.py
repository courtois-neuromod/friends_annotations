...
TRANSCRIPT_SCHEMA = json.load(open(os.path.join(this_dir, "transcript_schema.json")))
ALIGNMENT_SCHEMA = json.load(open(os.path.join(this_dir, "alignment_schema.json")))
        ...
        for w_idx, wrd in enumerate(txt):
            ...
            for wrd2 in new_up_wrd:
                ...
                if len(between) != 0:
                    ...
                    #     int(wrd2)
                    #     numwrd
    f.close()s = infl.number_to_words(wrd2, andword='')
