...
def remove_charac_and_split(words):
    ...
    if ':' in sentence_clean:
        tmp = sentence_clean.split(': ')
        sentence_clean = tmp[-1]
        speaker = tmp[0]
    ...
    while "<" in sentence_clean:
        ...
        i2 = sentence_clean.index(">")
        sentence_clean = sentence_clean[:i1] + sentence_clean[i2+1:]
    return sentence_clean.strip().split(), speaker
...
def list_first_words_from_subtitles(subtitles_file):
    ...
    for line in subtitles:
        ...
        if line.isdigit():
            ...
            sentences[-1][1] += line
    list_first_words = [[sentence[0], remove_charac_and_split(sentence[1])[0]] for sentence in sentences]
...
def get_timestamp(subtitles_file, audio_file, shift, path):
    ...
    for sentence in sentences:
        ...
        if start_time > 0 and end_time < len(sound):
            words, speaker = remove_charac_and_split(sentence[2])
            if speaker: # if speaker isn't empty, then update current_speaker
            ...
            if len(words) > 0:
                ...
                for i in range(len(words)):
                    ...
                    w.append(words[i])
                    to_write += str(timestamp[i]) + ', ' + str(current_speaker) + ': ' + words[i] + '\n'
                f = open(path+ '.txt',"a")
                f.write(to_write + '\n')
                f.close()
    d = {'start_time': s, 'end_time': e, 'word': w}
